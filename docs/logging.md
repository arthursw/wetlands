...
Process logs in real-time with custom logic (parsing, metrics, analytics):
...
class LogAnalyzer:
    ...
    def on_log(self, record: logging.LogRecord):
        ...
        if "Installing" in message:
            ...
            if match:
                self.packages_installed.append(match.group(1))
                print(f"📦 Package: {match.group(1)}")
...
1. **Use `logger.logger`** to access handler methods (not `logger.addHandler()`)
