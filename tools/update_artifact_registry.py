...
def generate_registry(
    ...
    return render_registry(resolved_pixi_version, pixi_hashes)
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--latest",
        action="store_true",
        help="Resolve and use the latest stable Pixi release.",
    )
    parser.add_argument(
        "--pixi-version",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.latest:
        if args.pixi_version is not None:
            parser.error("--latest cannot be combined with --pixi-version.")
        pixi_version = "latest"
    else:
        if args.pixi_version is None:
            parser.error("provide --pixi-version, or use --latest.")
        pixi_version = args.pixi_version

    try:
        generated = generate_registry(pixi_version)
