...
from cema.dependencies import Dependencies
...
class EnvironmentManager:
    ...
    def _configFileOption(self, doubleQuotes=False):
    ...
    def _getOutput(
        self, process: subprocess.Popen, commands: list[str], log=True, strip=True
    ):
    ...
    def dependenciesAreInstalled(self, environment: str, dependencies: Dependencies):
        ...
        if hasCondaDependencies:
            if "conda" not in installedDependencies:
                installedDependencies["conda"], _ = self.executeCommands(
                    self._activateConda()
                    + [
        ...
        if "pip" not in installedDependencies:
            if environment is not None:
                installedDependencies["pip"], _ = self.executeCommands(
                    self._activateConda()
    ...
    def _formatDependencies(
        ...
        raiseIncompatibilityError=True,
    ):
        dependencies = (
            dependencies[package_manager] if package_manager in dependencies else []
        )
        finalDependencies = []
    ...
    def _getCommandsForCurrentPlatfrom(
    ...
    def create(
        ...
        if self.environmentExists(environment):
            if errorIfExists:
                ...
                return True
        pythonVersion = (
            str(dependencies["python"]).replace("=", "")
        ...
        createEnvCommands += self._getCommandsForCurrentPlatfrom(
    ...
    def logOutput(self, process: subprocess.Popen, stopEvent: threading.Event):
        try:
            for line in iter(
