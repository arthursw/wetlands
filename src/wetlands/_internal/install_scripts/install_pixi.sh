...
__wrap__() {
    ...
    CURL_OPTIONS="${CURL_OPTIONS} --proxy ${PROXY_STRING}"
    WGET_OPTIONS="${WGET_OPTIONS} --proxy ${PROXY_STRING}"
    ...
    EXPECTED_HASH="$(cut -d' ' -f1 < "./checksums/$ARCHIVE_NAME.sha256")"
