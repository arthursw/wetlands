...
from cema.environment import ClientEnvironment, DirectEnvironment

def test_client_environment_initialization():
    process_mock = Mock(spec=Popen)
    env = ClientEnvironment("test_env", 5000, process_mock)
    assert env.name == "test_env"
    assert env.port == 5000
    assert env.process == process_mock
    assert env.connection is None

def test_client_environment_initialize():
    with patch("cema.environment.Client", autospec=True) as client_mock:
        ...
        env = ClientEnvironment("test_env", 5000, Mock(spec=Popen))
        env.initialize()
        
        assert env.connection == connection_mock
        client_mock.assert_called_once_with(("localhost", 5000))

def test_client_environment_execute_with_no_connection():
    env = ClientEnvironment("test_env", 5000, Mock(spec=Popen))
...
def test_client_environment_execute_with_mock_connection():
    env = ClientEnvironment("test_env", 5000, Mock(spec=Popen))
    connection_mock = Mock(spec=Connection)
    ...
    result = env.execute("test_module.py", "test_function", [1, 2, 3])
    assert result == 42
    connection_mock.send.assert_called_with({"action": "execute", "modulePath": "test_module.py", "function": "test_function", "args": [1, 2, 3]})

def test_client_environment_execute_with_error():
    env = ClientEnvironment("test_env", 5000, Mock(spec=Popen))
    connection_mock = Mock(spec=Connection)
    connection_mock.closed = False
    env.connection = connection_mock
    ...
    connection_mock.recv.side_effect = [
        {"action": "error", "message": "Test error"}
    ]
    
    with pytest.raises(ExecutionException):
        env.execute("test_module.py", "test_function", [1, 2, 3])

def test_client_environment_exit():
    env = ClientEnvironment("test_env", 5000, Mock(spec=Popen))
    connection_mock = Mock(spec=Connection)
    connection_mock.closed = False
    env.connection = connection_mock
...
def test_direct_environment_execute():
    env = DirectEnvironment("test_env")
...
def test_direct_environment_execute_function_not_found():
    env = DirectEnvironment("test_env")
    with patch("cema.environment.import_module") as import_mock:
        module_mock = Mock(spec={})
        import_mock.return_value = module_mock
        
        with pytest.raises(Exception, match="Module test_module has no function test_function."):
