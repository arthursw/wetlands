...
def test_format_dependencies(mock_settings_manager):
    ...
    dependencies: Dependencies = {
        ...
        "conda": [
            "numpy",
            {"name": "tensorflow", "platforms": ["linux-64"], "optional": False, "dependencies": True},
            {"name": "pandas", "platforms": ["win-64", "osx-64"], "optional": True, "dependencies": True},
        ],
    }
...
def test_get_install_dependencies_commands(mock_settings_manager):
    ...
    assert any(f"{mock_settings_manager.condaBin} install \"numpy\" -y" in cmd for cmd in commands)
