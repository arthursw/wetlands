...
def test_get_install_conda_commands_windows(mock_platform, command_generator, mock_settings_manager):
    ...
    assert any(re.match(rf'\.\\{condaBinPath} shell hook -s powershell \| Out-String \| Invoke-Expression', cmd) for cmd in commands)
...
@pytest.mark.parametrize("additional_commands, expected", [
    ({"all": ["common_cmd"], "linux": ["linux_cmd"], "windows": ["win_cmd"]}, ["common_cmd", "linux_cmd"]),
    ({"windows": ["win_cmd"]}, []),
